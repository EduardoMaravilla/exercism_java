...
class WordSearcher {
        ...
        for (String word : words) {
            ...
            if(palabra==false){
                 // Búsqueda diagonal
                       for (int i = 0; i < fila; i++) {
                      for (int j = 0; j < columna; j++) {
            // Buscar hacia la derecha y hacia abajo
    ...
    // Verificar si la palabra cabe dentro de la matriz
    int len = word.length();
